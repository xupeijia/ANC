...
# file_path = 'F:/grade_three/HMM/DaNN2/data/data2/1-signal-5120'
file_path = 'F:/grade_three/CWRU-5120-2/'
flower_class = [cla for cla in os.listdir(file_path) if ".txt" not in cla]

# 创建 训练集train 文件夹，并由5种类名在其目录下创建5个子目录
mk_file('CWRU-5120-2/train')
for cla in flower_class:
    mk_file('CWRU-5120-2/train/' + cla)

# 创建 验证集val 文件夹，并由5种类名在其目录下创建5个子目录
mk_file('CWRU-5120-2/val')
for cla in flower_class:
    mk_file('CWRU-5120-2/val/' + cla)
...
for cla in flower_class:
    ...
    for index, image in enumerate(images):
        ...
        if image in eval_index:
            ...
            new_path = 'CWRU-5120-2/val/' + cla
            copy(image_path, new_path)  # 将选中的图像复制到新路径

        # 其余的图像保存在训练集train中
        else:
            image_path = cla_path + image
            new_path = 'CWRU-5120-2/train/' + cla
